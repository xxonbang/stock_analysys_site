...
**Google 뉴스 검색 기반 재료분석 (필수)**:
- 각 분석 종목에 대해 Google 검색을 수행하여 최근 금융/경제 관련 뉴스를 최소 10건 수집하십시오.
- 검색 시 "[종목명/티커] 주가", "[종목명] 실적", "[종목명] 뉴스", "[종목명] 전망" 등 금융 키워드를 다양하게 조합하십시오.
- 수집된 뉴스 각각의 내용을 면밀히 분석하여 아래 분류 기준으로 **재료분석**을 실시하십시오:
  1. **호재(Positive Catalyst)**: 실적 호조, 신사업/신제품, 수주·계약 확대, 정책 수혜, 배당 확대, 자사주 매입, 목표가 상향, 업종 호황 등
  2. **악재(Negative Catalyst)**: 실적 부진, 규제 강화, 소송·과징금, 경영진 리스크, 공급망 이슈, 목표가 하향, 업종 침체 등
  3. **모니터링 이슈**: M&A 루머, 지배구조 변화, 업종 재편, 정책 변화 대기 등 방향성 미확정 이슈
- 각 재료의 **영향 시간축**(단기 1주 이내 / 중기 1~3개월 / 장기 3개월 이상)과 **영향 강도**(상·중·하)를 평가하십시오.
- 재료분석 결과를 리포트의 다른 섹션(현재 시장 상황, 기술적 분석 해석, 향후 전망 시나리오, 투자 의견)에 유기적으로 반영하십시오.
...
4. **재료분석 필수**: 각 종목에 대해 Google 검색으로 최근 금융 뉴스 10건을 수집하고, 호재/악재/모니터링 이슈로 분류하여 재료분석을 실시하세요. 재료분석 결과는 "재료분석 (뉴스 기반)" 섹션에 작성하고, 향후 전망과 투자 의견에도 반영하세요.
